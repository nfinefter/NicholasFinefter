...
import type { Playlist, PlaylistType } from '@/data/portfolio'
...
function featuredLabel(type: PlaylistType): string {
  if (type === 'experience') return 'Featured experience'
  if (type === 'website') return 'Featured case study'
...
export default function FeaturedBanner({ project, className }: FeaturedBannerProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-10 flex flex-col gap-6 p-6 md:flex-row md:items-center md:p-10">
          <div className="flex h-24 w-24 shrink-0 items-center justify-center overflow-hidden rounded-lg shadow-lg md:h-32 md:w-32">
            <PlaylistCover item={project} iconClassName="h-14 w-14 md:h-16 md:w-16" />
          </div>
          <div className="min-w-0 flex-1">
            <span className="mb-2 inline-block text-xs font-semibold uppercase tracking-widest text-white/70">
              {featuredLabel(project.type)}
            </span>
